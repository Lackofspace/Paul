...
std::streamsize FileInputStream::ReadFile(void *dstBuffer) {
    auto length = GetStreamSize();

    return ReadBlock(dstBuffer, length);
}

std::streamsize FileInputStream::GetStreamSize() {
    m_inputFile.seekg (0, std::ios::end);
    std::streamsize length = m_inputFile.tellg();
